...
import { FormControl, FormControlError, FormControlErrorIcon, FormControlLabel } from '../ui/form-control';
...
export const Input = (props: CustomInputProps & TextInputProps & IInputFieldProps) => {
  ...
  const InputContent = (
    <GInput
      ...
      style={[
        ...
        styles.input,
        {
          borderColor: props.isDisabled ? Colors.GRAY : error ? Colors.ERROR : Colors.LIGHT_GRAY,
    ...
    >
      ...
      {rightIcon && (
        <InputSlot className="pr-3" onPress={() => (props.secureTextEntry ? setSecureTextEntry(!secureTextEntry) : {})}>
          <InputIcon as={icon ? icon : secureTextEntry ? EyeIcon : EyeOffIcon} color={Colors.BLACK} size="lg" />
        </InputSlot>
      )}
    </GInput>
  );
  ...
  return (
    <FormControl isInvalid={!!error && touched} className={stretch ? 'flex-1' : ''}>
      {label && (
        <FormControlLabel>
          <Text color={Colors.BLACK} fontWeight={400}>
            {label}
          </Text>
        </FormControlLabel>
      )}
      ...
      {touched && error && (
        <FormControlError className="mt-2">
          ...
          <Text color={Colors.ERROR} fontWeight={300}>
